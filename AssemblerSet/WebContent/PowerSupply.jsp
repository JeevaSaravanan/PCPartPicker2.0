...
<nav class="navbar navbar-light navbar-expand-lg fixed-top shadow-sm bg-white"><a href="Home" class="navbar-brand">PC Part Picker</a>
   ...
   <section class="bg-light">
    <div class="container">
      <div class="row">
      <center><h1 style="text-align: center">Power Supply</h1></center>
      </div>
    </div>
  </section>
  <br>
...
<c:choose>  
  ...
  <tbody>
  <c:forEach items="${PowerSupply}" var="powerSupply">
  <tr>
  <td><img alt="powersupply" src="${powerSupply.value.getImage() }" width="80" height="80"></td>
  <td><c:out value="${powerSupply.value.getBrand()}"></c:out>&nbsp;</td>
  <td><c:out value="${powerSupply.value.getModel()}"></c:out></td>
  <td><c:out value="${powerSupply.value.getRate()}"></c:out></td>
  <td><a class="btn btn-primary" href="Home?page=addToCart&product=power-supply&item=<c:out value="${powerSupply.key}"/>">Add to Cart</a><td>
  </tr>
  </c:forEach>
  </tbody>
  </table>
