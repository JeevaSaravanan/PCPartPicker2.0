...
<nav class="navbar navbar-light navbar-expand-lg fixed-top shadow-sm bg-white"><a href="Home" class="navbar-brand">PC Part Picker</a>
   ...
   <section class="bg-light">
    <div class="container">
      <div class="row">
      <center><h1 style="text-align: center">Cabinets</h1></center>
      </div>
    </div>
  </section>
  <br>
...
<c:choose>  
  ...
  <tbody>
  <c:forEach items="${Cabinets}" var="cabinets">
  <tr>
  <td><img alt="cabinets" src="${cabinets.value.getImage() }" width="80" height="80"></td>
  <td><c:out value="${cabinets.value.getBrand()}"></c:out>&nbsp;</td>
  <td><c:out value="${cabinets.value.getModel()}"></c:out></td>
  <td><c:out value="${cabinets.value.getRate()}"></c:out></td>
  <td><a class="btn btn-primary" href="Home?page=addToCart&product=cabinets&item=<c:out value="${cabinets.key}"/>">Add to Cart</a><td>
  </tr>
  </c:forEach>
  </tbody>
  </table>
